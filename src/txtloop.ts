...
export class AalamTxtloop extends LitElement {
    ...
    private _showEl(el:HTMLElement) {
        ...
        const keyframes = this.getKeyFrames(this.show_animation_props.name) as Keyframe[];
    ...
    private _hideEl(el:HTMLElement) {
        const keyframes = this.getKeyFrames(this.hide_animation_props.name) as Keyframe[];
    ...
    private switch() {
        ...
        this.dispatchEvent(
            new CustomEvent("change",{detail:{nxt_index}}));
    }
    ...
    private adjustcontainerwidth(hiding_ix:number, show_rect:DOMRect) {
        ...
        if(show_rect != null && hide_rect != null && this._container_el != null)
        {
            this._container_el.style.width =
            `${Math.max(show_rect.width, hide_rect.width)}px`;
            this._container_el.style.height =
                `${Math.max(show_rect.height, hide_rect.height)}px`;
        }
    }
}
