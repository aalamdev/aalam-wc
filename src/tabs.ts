...
<div @click=${this._titleClicked}
     @animationend=${ (event:any) => this._animationEndEvent(event)}>
    <slot name='acc'></slot>
</div>`
        } else {
...
<div style=${this._internal_fashion=='column'?
    ...
    <div id="body-tab" @animationend=${ (event:any) => this._animationEndEvent(event)}>
...
<style>
    ...
    private _animationEndEvent(event:Event) {
