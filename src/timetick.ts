...
<div>
    ...
    private timeCount() {
        ...
        const variables = ['Seconds', 'Minutes', 'Hours', 'Date', 'Month', 'Year'];
        const var_limits = [60, 60, 24, new Date(now.getFullYear(), now.getMonth() + 1, 0).getDate(), 12,-1];
        ...
        if (!this._month_el) {
            ...
            components[3] = Math.floor((this._elapsed_date.getTime() - now.getTime())/(1000 * 24 * 3600));
        } else {
        ...
        if (this._week_el) {
            ...
            components[3] = components[3] + (weeks * 7);
