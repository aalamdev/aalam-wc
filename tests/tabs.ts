...
describe('aalam-tabs', () => {
    const validate_el = (el:HTMLElement, ix:number) => {
        ...
        if(title[ix].classList.contains(el.activecls))
            count++;
        if(el._animation_styles.open) {
            if(body[ix].classList.contains(
                   `__tab-anim-open-${el._anim_number}`)) {
                count++;
            }
        } else if(el.animationDur == 0 || (
                    el._animation_styles.close &&
                    !el._animation_styles.open)) {
            count++;
        }
        if(count == 4)
            return true;
    ...
    const validate_prev = (el:HTMLElement, prev_ix:number) => {
        ...
        if((el._animation_styles.open ||
            el._animation_styles.close) && el.animationDur) {
            expect(body[prev_ix].classList.contains(
                `__tab-anim-close-${el._anim_number}`)).to.equal(true);
            expect(body[prev_ix].classList.contains(
                `__tab-anim-open-${el._anim_number}`)).to.equal(false);
        }
    }
    ...
    it('animation', async () => {
        ...
        await new Promise((resolve) => {setTimeout( () => {
            resolve()}, tabs.animationDur)});
        expect(validate_el(tabs, 2)).to.equal(true);
        body[0].dispatchEvent(new Event('animationend', {bubbles: true}));
        validate_prev(tabs, 0);
        ...
        await new Promise((resolve) => {setTimeout( () => {
            resolve()}, tabs.animationDur)});
        expect(validate_el(tabs, 3)).to.equal(true);
        body[2].dispatchEvent(new Event('animationend', {bubbles: true}));
        validate_prev(tabs, 2);
