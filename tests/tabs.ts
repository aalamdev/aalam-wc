...
describe('aalam-tabs', () => {
    const validate_el = (el:HTMLElement, ix:number) => {
        let title = document.querySelectorAll("[slot=tab-title]");
        let body = document.querySelectorAll("[slot=tab-body]");
        let count = 0;
        ...
        if(title[ix].classList.contains(el.activecls))
            count++;
        if(el.animation) {
            if(body[ix].classList.contains('animation-open'))
                count++;
        }
        else if(el.animationDur == 0)
            count++;
    ...
    const fashion_style = (el:HTMLElement) => {
        let val = el._checkFashion();
        el._changeFashionStyle(val);
    }
    it('is defined', async () => {
    ...
    it('row-column-accordion', async () => {
        const el = await fixture (`
            <div id='parent'>
                <aalam-tabs fashion="xs:accordion;m:row;xl:column">
                    <div slot="tab-title" id="t1">title1</div>
                    <div slot="tab-body" id='b1'>body1</div>
                    <div slot='tab-title' class='tab-active' id='t2'>title2</div>
                    <div slot='tab-body' id='b2'>body2</div>
                    <div slot="tab-title" class='tab-active' id="t3">title3</div>
                    <div slot="tab-body" id='b3'>body3</div>
                    <div slot='tab-title' class='tab-active' id='t4'>title4</div>
                    <div slot='tab-body' id='b4'>body4</div>
                </aalam-tabs>
            </div>
        `);
        ...
        tabs.setAttribute("closize", col);
        expect(tabs._column_size.title).to.equal('30%');
        expect(tabs._column_size.body).to.equal('70%');
        tabs.attributeChangedCallback('colsize', '', col);
    ...
    it('animation', async () => {
        ...
        expect(tabs._animation_styles.close).to.equal('r2l');
        ...
        tabs.attributeChangedCallback('animation','', "show:l2r;hide:fade");
        await setViewport({width:1680, height:640});
        ...
        await setViewport({width:1000, height:640});
        tabs.attributeChangedCallback('animation', '',  "show:t2b");
        fashion_style(tabs);
        expect(tabs._internal_fashion).to.equal('row');
        expect(tabs._animation_styles.open).to.equal('t2b');
